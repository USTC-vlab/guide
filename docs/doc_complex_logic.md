...
示例：  
$readmemb 的使用先在 Verilog 代码目录下准备一个文件 file1.txt，存入数据：1111 1010 0101 1x1z 1_1 1111    或者 11111010 0101 1x1z 1_1_1_111 存在一行每个用空格隔开，跟分行存，输出结果是一样的，但是若在一行中不用空格隔开会出错，编译器会试图把一整行数据存在一个四位的存储单元中。  
